...
int main(int argc, const char **argv) 
{
	...
	sleep(5e2);
	printf(1, "parent: sending pause signal to child ret: %d \n", ret);
	signalProcess(ret, SIGNAL_PAUSE);
	
	sleep(5e2);
	printf(1, "parent: sending continue signal to child ret: %d \n", ret);
	signalProcess(ret, SIGNAL_CONTINUE);
	
	sleep(5e2);
	printf(1, "parent: sending kill signal to child ret: %d \n", ret);
