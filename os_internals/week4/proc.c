...
exit(void)
{
  ...
  // Pass abanoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->parent == curproc){
...
void get_siblings(void)
{
	struct proc *p; 
	struct proc *pt = myproc();
	int parent_pid = (pt->parent)->pid;

	acquire(&ptable.lock);
	for (p=ptable.proc; p<&ptable.proc[NPROC]; p++)
	{
		if (((p->parent)->pid == parent_pid)&&(p->pid != pt->pid))
		{
			cprintf("%d %s\n",p->pid, pstates[p->state]);
		}
	}
	release(&ptable.lock);
}
...
int numpp() //change istg
{
	struct proc *p = myproc();
	return find_num_pp(p->pgdir, p->sz);
}
...
int mmap(int bytes)
{
	...
	{
		cprintf("mod:%d pg size:%d bytes:%d\n",bytes%PGSIZE, PGSIZE, bytes);
		return 0;
	}
	p->sz += bytes;
	return old;
